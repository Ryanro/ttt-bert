...
from pytorch_pretrained_bert.modeling import (
    ...
    load_from_adapter,BERT_CONFIG_NAME
)
from pytorch_pretrained_bert.file_utils import PYTORCH_PRETRAINED_BERT_CACHE, CONFIG_NAME
...
def load_bert(task_type, bert_model_name, bert_load_mode, all_state, num_labels,
    ...
    if bert_config_json_path is None:
        bert_config_json_path = os.path.join(get_bert_config_path(bert_model_name), CONFIG_NAME)
        if not os.path.exists(bert_config_json_path):
            # Backward compatibility with old naming format
            bert_config_json_path = os.path.join(get_bert_config_path(bert_model_name), BERT_CONFIG_NAME)
